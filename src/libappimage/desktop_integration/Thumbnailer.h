...
namespace appimage {
    namespace desktop_integration {
        ...
        public:
            explicit Thumbnailer(const std::string& appImagePath, const std::string& xdgCacheHome = "");
            ...
            boost::filesystem::path getNormalThumbnailPath(std::string canonicalPathMd5) const;

            boost::filesystem::path getLargeThumbnailPath(std::string canonicalPathMd5) const;

            std::string getCanonicalPathMd5() const;

        private:

            DesktopIntegrationResources extractResources() const;
